					...
					<TableBody>
						{userOrders.map((order) => (
							<TableRow key={order.id}>
								...
								<TableCell>
									<Badge className={order.status === "succeeded" ? "bg-green-600 hover:bg-green-700" : "bg-secondary-foreground"}>{order.status}</Badge>
								</TableCell>
													...
													<TableBody>
														{order.orderProduct.map(({product, productVariants, quantity}) => (
															<TableRow key={product.id}>
																...
																<TableCell>
																	<div className="w-4 h-4 rounded-full" style={{background: productVariants.color}} />
																</TableCell>
																<TableCell>
																	{quantity}
																</TableCell>
