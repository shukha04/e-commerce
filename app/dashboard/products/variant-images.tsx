			...
			<FormField
				...
				render={({ field }) => (
					<FormItem>
						...
						<FormControl>
							<UploadDropzone
								className="ut-allowed-content:text-secondary-foreground ut-label:text-primary ut-upload-icon:text-primary/50 hover:bg-primary/10 transition-all duration-200 ease-in-out border-secondary ut-button:bg-primary/75 ut-button:ut-readying:bg-primary/50"
								onUploadError={(error) => {
						...
						{fields.map((field, index) => {
							return (
								<Reorder.Item
									...
									value={field}
									className={cn(field.url.search("blob:") === 0 ? "animate-pulse transition-all" : "", "text-sm font-bold text-muted-foreground hover:text-primary")}>
